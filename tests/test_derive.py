...
import data
from models.core.data import derive
...
def test_genomic_windows(chromsizes, window, step, exclude, expected):
    if exclude:
    ...
    result = list(derive.genomic_windows(chromsizes, window, step, exclude, dropshort=False))
    assert expected == result

    expected = [exp for exp in expected if exp.length == window]
    result = list(derive.genomic_windows(chromsizes, window, step, exclude, dropshort=True))
    assert expected == result
